...
class EnginesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create engine" do
    assert_difference('Engine.count') do
      post engines_url, params: { engine: { astronauts: @engine.astronauts, model: @engine.model, name: @engine.name } }
    end
  ...
  test "should update engine" do
    patch engine_url(@engine), params: { engine: { astronauts: @engine.astronauts, model: @engine.model, name: @engine.name } }
